...
import type { Department, Paginated } from '../api/departments'
...
export default function DepartmentList() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-semibold text-white mb-6">Departments</h1>
      {data && (
        <>
              ...
              <tbody>
                {data.items.map((d) => (
                  <tr key={d.id} className="border-b border-[var(--border)] last:border-0">
                    ...
                    <td className="px-4 py-3 text-[var(--muted)]">{d.code ?? '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
